...
def load_data(con, data_dir: Path):
    ...
    if csv_files:
        print(f"[BASELINE] Loading {len(csv_files)} CSV parts from {data_dir} ...")
        con.execute(f"""
            ...
            FROM casted;
        """)
        print(f"[BASELINE] Loading complete")
    else:
...
def run(queries, data_dir: Path, out_dir: Path):
    ...
    for i, q in enumerate(queries, 1):
        ...
        print(f"[BASELINE] Rows: {len(rows)} | Time: {dt:.3f}s")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("./out"),
        help="Where to output query results-full (default: ./out)"
    )

    args = parser.parse_args() #args package
